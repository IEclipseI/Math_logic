...
happyExpList = Happy_Data_Array.listArray (0,40) ([12544,12544,1024,2048,0,0,0,12544,12544,0,1536,12544,12544,16384,0,12544,0,0,2048,0,0
	])

{-# NOINLINE happyExpListPerState #-}
happyExpListPerState st =
    token_strs_expected
  where token_strs = ["error","%dummy","%start_parse","Expr","Disj","Conj","Neg","Term","VAR","IMPL","OR","AND","NOT","LEFTB","RIGHTB","%eof"]
        bit_start = st * 16
        bit_end = (st + 1) * 16
...
happyReduction_10 (HappyTerminal (VarL happy_var_1))
	 =  HappyAbsSyn8
		 (Var happy_var_1
	)
...
happyNewToken action sts stk (tk:tks) =
	...
	case tk of {
	VarL happy_dollar_dollar -> cont 9;
	ImplicationL -> cont 10;
	OrL -> cont 11;
	AndL -> cont 12;
	NotL -> cont 13;
	LeftBracketL -> cont 14;
	RightBracketL -> cont 15;
	_ -> happyError' ((tk:tks), [])
	}
